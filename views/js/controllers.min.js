...
$(".list-edit-btn").click(function(){var $listSaveBtn,$listNewBtn,$btn=$(this),uri=$btn.data("uri"),$listContainer=$("#list-data_"+uri),$listForm=$listContainer.find("form"),$listTitleBar=$listContainer.find(".container-title h6"),$listToolBar=$listContainer.find(".data-container-footer");if(!$listForm.length){$listForm=$("<form>"),$listContainer.wrapInner($listForm),$listContainer.find("form").append('<input type="hidden" name="uri" value="'+uri+'" />');var $labelEdit=$("<input type='text' name='label' value=''/>").val($listTitleBar.text());$listTitleBar.html($labelEdit),$listContainer.find(".list-element").length&&$listContainer.find(".list-element").replaceWith(function(){return $("<input type='text' name='"+$(this).attr("id")+"' value='' />").val($(this).text())
...
$.postJson(delListUrl,{uri:uri},function(response){response.deleted?(feedback().success(__("List deleted")),$list.remove()):feedback().error(__("List not deleted"))})}})}}}),define("css!taoBackOffice/lib/vis/vis.min",[],function(){}),define("taoBackOffice/treeRender",["jquery","lodash","taoBackOffice/lib/vis/vis.min","css!taoBackOffice/lib/vis/vis.min"],function($,_,vis){"use strict";function destroy(){null!==network&&(network.destroy(),network=null)}var network=null,treeContainer=null,settings=null,data=null,treeRender={init:function(container,treeData,options){if(!container instanceof Element)throw new TypeError("tree container must be specified");treeContainer=container,options=options||{},treeData=treeData||{nodes:[],edges:[]},settings={layout:{hierarchical:{sortMethod:"directed",levelSeparation:200}},nodes:{mass:1,shape:"box",color:{border:"#222",background:"#f2f0ee",highlight:{border:"#222",background:"#f2f0ee"}},font:{face:"'Franklin Gothic', 'Franklin Gothic Medium', 'Source Sans Pro', sans-serif",color:"#222"}},edges:{smooth:!1,width:.2,color:{color:"#222"},arrows:{to:!0},physics:!1,scaling:{label:!1}}},$.extend(settings,options),data={nodes:treeData.nodes?treeData.nodes:[],edges:treeData.edges?treeData.edges:[]}
},run:function(){destroy(),network=new vis.Network(treeContainer,data,settings),network.once("initRedraw",function(){data.nodes.length>100&&(network.setOptions($.extend(settings,{physics:{hierarchicalRepulsion:{nodeDistance:200},stabilization:{fit:!1}}})),network.fit({nodes:[data.nodes[0].id,data.nodes[1].id],animation:{duration:400,easingFunction:"linear"}}))}),window.network=network,window.d=data}};return treeRender}),define("taoBackOffice/controller/main/view",["layout/actions/binder","uri","jquery","context","helpers","taoBackOffice/treeRender"],function(binder,uri,$,context,helpers,treeRender){"use strict";var itemRunnerController={start:function(){var $container=$("#tree-container"),uri=$container.data("uri");
