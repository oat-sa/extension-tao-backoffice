define("taoBackOffice/controller/routes",[],function(){"use strict";return{Lists:{actions:{index:"controller/list/index"}},Trees:{actions:{viewTree:"controller/tree/view"}}}}),define("taoBackOffice/controller/list/index",["jquery","i18n","helpers","ui/feedback"],function($,__,helpers,feedback){"use strict";function _addSquareBtn(title,icon,$listToolBar){var $btn=$("<button>",{"class":"btn-info small lft icon-"+icon,title:__(title)});return $listToolBar.append($btn),$btn}return{start:function(){var saveUrl=helpers._url("saveLists","Lists","taoBackOffice"),delListUrl=helpers._url("removeList","Lists","taoBackOffice"),delEltUrl=helpers._url("removeListElement","Lists","taoBackOffice");
...
return $("<input type='text' name='"+$(this).attr("id")+"' value='' />").val($(this).text())});var elementList=$listContainer.find("ol");elementList.addClass("sortable-list"),elementList.find("li").prepend('<span class="icon-grip" ></span>'),elementList.find("li").append('<span class="icon-checkbox-crossed list-element-delete-btn" style="cursor:pointer;" ></span>'),elementList.sortable({axis:"y",opacity:.6,placeholder:"ui-state-error",tolerance:"pointer",update:function(event,ui){var map={};$.each($(this).sortable("toArray"),function(index,id){map[id]="list-element_"+(index+1)}),$(this).find("li").each(function(){var id=$(this).attr("id");if(map[id]){$(this).attr("id",map[id]);
var newName=$(this).find("input").attr("name").replace(id,map[id]);$(this).find("input").attr("name",newName)}})}}),$listSaveBtn=_addSquareBtn("Save element","save",$listToolBar),$listSaveBtn.on("click",function(){return $.postJson(saveUrl,$(this).closest("form").serializeArray(),function(response){response.saved?(feedback().success(__('"list saved"')),helpers._load(helpers.getMainContainerSelector(),helpers._url("index","Lists","taoBackOffice"))):feedback().error(__("List not saved"))}),!1}),$listNewBtn=_addSquareBtn("New element","add",$listToolBar),$listNewBtn.click(function(){var level=$(this).closest("form").find("ol").children().length+1;return $(this).closest("form").find("ol").append("<li id='list-element_"+level+"'><span class='icon-grip' ></span><input type='text' name='list-element_"+level+"_' /><span class='icon-add list-element-delete-btn' ></span></li>"),
...
$.postJson(delListUrl,{uri:uri},function(response){response.deleted?(feedback().success(__("List deleted")),$list.remove()):feedback().error(__("List not deleted"))})}})}}}),define("css!taoBackOffice/lib/vis/vis.min",[],function(){}),define("taoBackOffice/treeRender",["jquery","lodash","taoBackOffice/lib/vis/vis.min","css!taoBackOffice/lib/vis/vis.min"],function($,_,vis){"use strict";function destroy(){null!==network&&(network.destroy(),network=null)}var network=null,treeContainer=null,settings=null,data=null,treeRender={init:function(container,treeData,options){if(!container instanceof Element)throw new TypeError("tree container must be specified");treeContainer=container,
options=options||{},treeData=treeData||{nodes:[],edges:[]},settings={layout:{hierarchical:{sortMethod:"directed",levelSeparation:200}},nodes:{shape:"box",color:{border:"#222",background:"#f2f0ee",highlight:{border:"#222",background:"#f2f0ee"}},font:{face:"'Franklin Gothic', 'Franklin Gothic Medium', 'Source Sans Pro', sans-serif",color:"#222"}},edges:{smooth:!1,width:.2,color:{color:"#222"},arrows:{to:!0},physics:!1,scaling:{label:!1}}},$.extend(settings,options),data={nodes:treeData.nodes?treeData.nodes:[],edges:treeData.edges?treeData.edges:[]}},run:function(){destroy(),network=new vis.Network(treeContainer,data,settings),network.once("initRedraw",function(){
